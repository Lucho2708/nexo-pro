...
use App\Modules\Asamblea\Events\VoteCast;
                    ...
                    $q->where("{$quorumTable}.user_id", $user->id)
                      ->orWhereExists(function ($query) use ($user, $quorumTable) {
                          $query->select(DB::raw(1))
                              ->from('property.unidad_user')
                              ->whereRaw("property.unidad_user.unidad_id = {$quorumTable}.unidad_id")
                              ->where('property.unidad_user.user_id', $user->id);
                      });
                })
                ->pluck('unidad_id')
                ->toArray();
