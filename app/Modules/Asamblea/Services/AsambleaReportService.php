...
class AsambleaReportService
{
    ...
    public function prepareReportData(Asamblea $asamblea): array
    {
        ...
        $participants = DB::table($tableName)
            ->join('iam.users', 'iam.users.id', '=', "{$tableName}.user_id")
            ->join('property.unidades', 'property.unidades.id', '=', "{$tableName}.unidad_id")
            ...
            ->select([
                ...
                'iam.users.name as user_name',
                'property.unidades.nombre as unit_name',
                'property.unidades.torre as unit_tower'
            ])
            ->get()
            ->unique('unidad_id');
